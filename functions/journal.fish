...
set data_dir
if test -n "$FISH_JOURNAL_DIR"
    set data "$FISH_JOURNAL_DIR"
else if test -n "$XDG_DATA_DIR"
    set data_dir "$XDG_DATA_DIR/fish_journal"
else
    set data_dir "$HOME/.local/share/fish_journal"
end
...
function __journal_new
    ...
    if not test -d "$data_dir"
        echo "Creating $data_dir to store journal entries"
        mkdir -p "$data_dir"
    end

    if test \( -d "$data_dir" \) -a \( -f "$data_dir" \)
        echo "$data_dir is a file, exiting"
        exit 1
    end
...
function __journal_list_tags
    ...
    # and skip duplicates
    set -l tags
    for line in (grep -hR "tags:" "$data_dir")
